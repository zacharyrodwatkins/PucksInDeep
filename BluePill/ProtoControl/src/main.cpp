...
void setup() {
  ...
  // }
  write_to_motor_simple((uint8_t) 64);
  delay(1000);
}
...
void loop() {
  write_to_motor_simple((uint8_t) 127);
  delay(100);
  return;
  readAngle(result);
  ...
  if (total_angles[1]<start_angles[1]+360){
    ...
    if (j!=-1){
      ...
      mean *= 1.0/num_not_zero;
      Serial.print(mean);
      Serial.print(" ");
      Serial.print(num_not_zero);
      Serial.print("\n");
      j = -1;
    }
  }
